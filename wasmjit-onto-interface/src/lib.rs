...
pub use wasmjit_capi::{
    ...
    wasmjit_bytes_new, wasmjit_bytes_t, wasmjit_chain_context_t, wasmjit_instance_invoke,
...
pub unsafe extern "C" fn ontio_debug(vmctx: *mut VMContext, data_ptr: u32, l: u32) {
    check_host_panic(|| {
        ...
        if res.kind != wasmjit_result_success {
            panic!(std::str::from_utf8(&bytes_to_boxed_slice(res.msg))
                .unwrap()
                .to_string());
        }
    })
...
pub unsafe extern "C" fn ontio_storage_read(
    ...
    check_host_panic(|| {
        ...
        if jit_u32.res.kind != wasmjit_result_success {
            panic!(std::str::from_utf8(&bytes_to_boxed_slice(jit_u32.res.msg))
                .unwrap()
                .to_string());
        }
        jit_u32.v
...
pub unsafe extern "C" fn wasmjit_read_memory(
    ...
    let result = catch_host_panic(|| {
        let vmctx_r = convert_vmctx(vmctx);
        let instance = (&mut *vmctx_r).instance();
        let memory = instance
            .memory_slice_mut(DefinedMemoryIndex::from_u32(0))
            .unwrap();
        if memory.len() < (data_ptr + data_len) as usize {
            panic!("wasmjit_read_memory access out of bound.");
        }
    ...
    match result {
        Ok(res) => res,
        Err(err_message) => wasmjit_result_t {
            kind: wasmjit_result_err_trap,
            msg: bytes_from_vec(err_message.as_bytes().to_vec()),
        },
    }
}
...
pub unsafe extern "C" fn wasmjit_write_memory(
    ...
    let result = catch_host_panic(|| {
        ...
        let memory = instance
            ...
            .unwrap();
        if memory.len() < (data_ptr + data_len) as usize {
            panic!("wasmjit_write_memory access out of bound.");
        }
...
pub unsafe extern "C" fn wasmjit_invoke(
    ...
    let mut instance = 0 as *mut wasmjit_instance_t;
    //let chain = unsafe { convert_chain_ctx(chainctx) };
    ...
    wasmjit_instance_invoke(instance, chainctx)
}
